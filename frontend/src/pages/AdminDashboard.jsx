    ...
    <div style={{ minHeight: "100vh", background: "#f5f6fa", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ background: NAV, padding: "28px 40px 24px", borderBottom: `3px solid ${ACCENT}` }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          ...
          <button onClick={handleLogout} style={{ background: "#ef4444", border: "none", color: "#fff", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'Segoe UI', sans-serif", marginTop: 4 }}
                            ...
                            <div style={{ padding: "18px 24px", flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                                  ...
                                  <button onClick={() => handleDeleteEvent(e._id, e.title)} style={{ background: "#fef2f2", border: "none", color: "#ef4444", borderRadius: 6, padding: "5px 14px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>🗑 Delete</button>
                                </div>
                              </div>
                              ...
                              <div style={{ display: "flex", gap: 20, fontSize: 12, color: "#475569", flexWrap: "wrap" }}>
                                <span>🏫 <strong>Committee:</strong> {e.organizer?.name || "—"}</span>
                                <span>📍 <strong>Venue:</strong> {e.venue}</span>
                                <span>👥 <strong>Capacity:</strong> {e.maxCapacity}</span>
                                {e.registrationDeadline && <span>⏳ <strong>Deadline:</strong> {fmt(e.registrationDeadline)}</span>}
                              </div>
                            </div>
                          </div>
                        );
              ...
              <div>
                          ...
                          <div key={p._id} style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 12, overflow: "hidden" }}>
                            ...
                            <div style={{ padding: "14px 16px" }}>
                              ...
                              <div style={{ display: "flex", gap: 8 }}>
                                <button onClick={() => handlePhotoAction(p._id, "approved")} style={{ flex: 1, padding: "8px 0", background: "#dcfce7", border: "none", color: "#15803d", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>✅ Approve</button>
                                <button onClick={() => handlePhotoAction(p._id, "rejected")} style={{ flex: 1, padding: "8px 0", background: "#fef2f2", border: "none", color: "#ef4444", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>❌ Reject</button>
                              </div>
                            </div>
                          </div>
                        ))}
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                    ...
                    <div>
                      {/* ── CHANGED: hidden file input; button now opens modal ── */}
                      <input ref={fileInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handleAdminPhotoUpload} />
                      <button onClick={handleAdminUploadClick} disabled={uploadingPhoto} style={{ background: ACCENT, border: "none", color: "#fff", borderRadius: 8, padding: "9px 20px", fontWeight: 700, fontSize: 13, cursor: uploadingPhoto ? "not-allowed" : "pointer", fontFamily: "'Segoe UI', sans-serif" }}>
                        ...
                        {galleryPhotos.map(p => (
                          <div key={p._id} style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 12, overflow: "hidden" }}>
                            ...
                            <div style={{ padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                              ...
                              <button onClick={() => handleDeleteGalleryPhoto(p._id)} style={{ background: "#fef2f2", border: "none", color: "#ef4444", borderRadius: 6, padding: "4px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>🗑</button>
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 18, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={{ background: adminPhotoDesc.trim() ? ACCENT : "#93c5fd", border: "none", color: "#fff", borderRadius: 8, padding: "9px 20px", fontWeight: 700, fontSize: 13, cursor: adminPhotoDesc.trim() ? "pointer" : "not-allowed", fontFamily: "'Segoe UI', sans-serif" }}
              >📸 Choose Photo</button>
            </div>
          </div>
        </div>
