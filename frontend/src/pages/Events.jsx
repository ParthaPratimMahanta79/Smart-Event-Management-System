...
// ── Must match the key used in Eventregister.jsx exactly ──────────────────────
...
function SectionHeading({ label, color, count }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
      ...
      <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#0f172a", fontFamily: "'Georgia', serif", letterSpacing: "-0.3px" }}>
        {label}
      </h2>
      <span style={{ background: color + "20", color, fontSize: 12, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>
        {count} Events
      </span>
    </div>
  );
...
function EventCard({ event, past, registeredIds }) {
  const [hov, setHov] = useState(false);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "16px 20px 20px" }}>
        ...
        <h3 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 700, color: "#0f172a", lineHeight: 1.35, fontFamily: "'Georgia', serif" }}>
          {event.title}
        </h3>
        <p style={{ margin: "0 0 14px", fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>{event.description}</p>
...
export default function Events() {
  ...
  // Reload registered IDs when user logs in/out or switches accounts
  useEffect(() => {
    setRegisteredIds(getRegisteredIds(user?.id));
  }, [user?.id]);

  // Reload registered IDs when returning from the registration page
  useEffect(() => {
    const refresh = () => setRegisteredIds(getRegisteredIds(user?.id));
      ...
      <div style={{ padding: "116px 40px 60px", maxWidth: 1200, margin: "0 auto", boxSizing: "border-box" }}>
        ...
        {ongoing.length > 0 && (
          <div style={{ marginBottom: 56 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 28 }}>
              {ongoing.slice(0, showOngoing).map(e => <EventCard key={e.id} event={e} registeredIds={registeredIds} />)}
            </div>
        ...
        {upcoming.length > 0 && (
          <div style={{ marginBottom: 56 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 28 }}>
              {upcoming.slice(0, showUpcoming).map(e => <EventCard key={e.id} event={e} registeredIds={registeredIds} />)}
            </div>
        ...
        {past.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 28 }}>
              {past.slice(0, showPast).map(e => <EventCard key={e.id} event={e} past registeredIds={registeredIds} />)}
            </div>
