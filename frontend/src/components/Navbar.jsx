...
const C = { navy: "#0d1b2a", navyDark: "#1a3557", blue: "#2563eb", muted: "#94a3b8" };
  ...
  return (
    <input type={type} value={value} onChange={onChange} placeholder={placeholder}
      ...
      style={{
        ...
        outline: "none", fontFamily: "'Segoe UI', sans-serif", transition: "border-color 0.15s",
      }}
    />
  );
}

// ── Exported so Gallery/Events can trigger the modal with optional redirect ───
export const openLoginModal = (redirectTo = null) => {
  window.dispatchEvent(new CustomEvent("open-login-modal", { detail: { redirectTo } }));
};
...
export default function Navbar() {
  ...
  useEffect(() => {
    const handler = (e) => {
      setPendingNav(e.detail?.redirectTo || null);
      setError(""); setForm({ email: "", password: "" });
      setLoginOpen(true);
    };
  ...
  const closeLogin = () => { setLoginOpen(false); setError(""); setPendingNav(null); };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        } else if (res.user.role === "committee") {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
          {links.map(({ id, label, fn }) => (
            <button key={id} onClick={fn} style={{
              ...
              fontFamily: "system-ui, sans-serif", transition: "color 0.2s", position: "relative",
            }}>
              {label}
              {active === id && <span style={{ position: "absolute", bottom: 3, left: "50%", transform: "translateX(-50%)", width: 20, height: 2.5, borderRadius: 2, background: C.blue, display: "block" }}/>}
            </button>
          ))}
          ...
          {isLoggedIn ? (
            <div style={{ position: "relative", marginLeft: 8 }} onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setDropOpen((v) => !v)} style={{
                ...
                fontFamily: "system-ui", fontSize: 14, fontWeight: 600, transition: "background 0.2s",
              }}
                onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.18)"}
                onMouseLeave={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.10)"}
              >
                <div style={{ width: 26, height: 26, borderRadius: "50%", background: C.blue, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700 }}>
                  {user?.name?.charAt(0).toUpperCase()}
                </div>
                {user?.name?.split(" ")[0]}
                <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{ transform: dropOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>
              ...
              {dropOpen && (
                <div style={{
                  ...
                  minWidth: 180, overflow: "hidden", border: "1px solid #e2e8f0", zIndex: 1000,
                }}>
                  <div style={{ padding: "12px 16px", borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <span style={{ fontSize: 10, background: "#f0f4ff", color: C.blue, borderRadius: 4, padding: "1px 6px", fontWeight: 700, textTransform: "capitalize" }}>{user?.role}</span>
                  </div>
                  {user?.role === "admin"     && <button onClick={() => { setDropOpen(false); navigate("/AdminDashboard"); }}     style={dropItemStyle}>🛡 Admin Dashboard</button>}
            ...
            <button
              onClick={() => { setError(""); setForm({ email: "", password: "" }); setPendingNav(null); setLoginOpen(true); }}
              style={{
                marginLeft: 8, padding: "7px 22px", background: C.blue, border: "none",
      ...
      {loginOpen && (
        <div onClick={closeLogin} style={{
          ...
          <div onClick={(e) => e.stopPropagation()} style={{
            background: "#fff", borderRadius: 20, boxShadow: "0 24px 64px rgba(0,0,0,0.22)",
            ...
            <button onClick={closeLogin} style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: "#64748b",
            }}>✕</button>

            <div style={{ textAlign: "center", marginBottom: 28 }}>
              <div style={{ width: 52, height: 52, borderRadius: 12, background: C.navyDark, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
            ...
            {error && <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#ef4444", fontWeight: 600, marginBottom: 18 }}>⚠ {error}</div>}
            ...
            <form onSubmit={handleLogin}>
              ...
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <button type="button" onClick={() => setShowPass((v) => !v)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#94a3b8", fontWeight: 600 }}>
                    {showPass ? "Hide" : "Show"}
                  </button>
                </div>
              </div>
              <button type="submit" disabled={loading} style={{
                width: "100%", padding: "13px 0", background: loading ? "#94a3b8" : C.navyDark,
                color: "#fff", border: "none", borderRadius: 8, fontWeight: 800, fontSize: 15,
                cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Segoe UI', sans-serif",
            ...
            <button onClick={() => { closeLogin(); navigate("/Register"); }} style={{
              width: "100%", padding: "12px 0", background: "transparent", border: `1.5px solid ${C.navyDark}`,
              borderRadius: 8, color: C.navyDark, fontWeight: 700, fontSize: 14, cursor: "pointer",
...
const dropItemStyle = { width: "100%", padding: "10px 16px", background: "none", border: "none", textAlign: "left", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#0f172a", fontFamily: "system-ui", display: "block" };
const labelStyle = { display: "block", fontSize: 12, fontWeight: 700, color: "#475569", marginBottom: 6, letterSpacing: "0.04em", textTransform: "uppercase" };